...
function Contact() {
  return (
    <section id='contact' className='relative bg-skin-primary w-full h-dvh'>
      ...
      <a href='#contact' className='sticky top-20 h-0 w-max text-skin-secondary before:inline-flex flex before:w-5 before:h-5 before:mr-6 before:mt-2 before:bg-skin-secondary before:rounded-full pl-16 p-3 text-3xl font-bold -ml-2 cursor-pointer z-[12] hover:text-skin-action'>
        Contact
      </a>
      <div className='relative'>
        <div className="w-full h-dvh flex justify-center flex-col items-center relative z-10 md:m-auto">
          ...
          <button className='bg-skin-secondary p-8 pt-3 pb-3 rounded-md text-skin-primary hover:bg-skin-secondary/80 active:scale-95'>Say Hello !!</button>
        </div>
      </div>
    </section>
  );
