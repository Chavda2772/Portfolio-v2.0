...
function Skills() {
  ...
  // Template
  return (
    <div id='skills' className='relative bg-skin-primary'>
          ...
          <div className="h-3/6 md:h-full bg-skin-secondary w-1 ml-1 absolute mt-8"></div>
          {
            skillsDetails.map((item, idx) => {
              ...
              if (item.name == selectedSkills.name)
                classList = 'before:inline-block before:w-5 before:h-5 before:mr-5 before:bg-skin-secondary before:rounded-full pl-0 p-3 text-3xl before:-ml-1 hover:cursor-pointer text-skin-action';
              else
          ...
          <div className='w-3/4 md:w-2/4 flex items-start justify-center flex-col rounded-2xl bg-skin-secondary'>
            {
              selectedSkills?.list?.map((item, idx) => {
                if (!item) return;
                return <h2 key={idx} className='text-2xl p-3 pl-6 text-skin-primary hover:cursor-pointer hover:text-skin-action'>{item}</h2>
              })
            }
          </div>
        </div>
